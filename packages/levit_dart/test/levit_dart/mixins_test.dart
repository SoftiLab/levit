...
void main() {
  ...
  group('LevitReactiveTasksMixin', () {
    ...
    test('updates tasks map state', () async {
      final completer = Completer();
      final future = controller.runTask('task1', () async {
        await completer.future;
        return 'done';
      });

      expect(controller.tasks['task1'], isA<AsyncWaiting>());

      completer.complete();
      await future;

      expect(controller.tasks['task1'], isA<AsyncSuccess>());
      expect((controller.tasks['task1'] as AsyncSuccess).value, 'done');
    });
    ...
    test('updates totalProgress', () async {
      ...
      controller.runTask('t1', () => c1.future, weight: 1.0);
      controller.runTask('t2', () => c2.future, weight: 1.0);
    ...
    test('clearTask removes task and cancels it', () async {
      final c = Completer();
      controller.runTask('t1', () => c.future);
    ...
    test('clearCompleted removes success/idle', () async {
      controller.tasks['s'] = AsyncSuccess(1);
      controller.tasks['i'] = AsyncIdle();
      controller.tasks['w'] = AsyncWaiting();
    ...
    test('onTaskError catches errors', () async {
      ...
      await controller.runTask('t1', () => throw 'fail');
      expect(controller.tasks['t1'], isA<AsyncError>());
      expect(caught, true);
    });
